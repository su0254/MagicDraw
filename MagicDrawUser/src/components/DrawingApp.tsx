...
// import { Box, Button, Slider, Typography, Input, Paper, Switch,
//   FormControlLabel,
//   IconButton,
//   Tooltip,
//   Grid,
...
const DrawingApp = () => {
  ...
  const fetchColorInstructions = async () => {
    if (backgroundImageUrl) {
      try {
        const response = await axios.post("http://127.0.0.1:5000/color_instructions", {
          image_path: backgroundImageUrl,
        })
        setInstructions(response.data)
      } catch (error) {
        console.error("Error fetching color instructions:", error)
        setInstructions("לא ניתן לטעון הוראות צביעה כרגע.")
      }
    }
  }
